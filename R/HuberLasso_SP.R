...
#'
#' @examples
HuberLasso_SP <- function(X, y, n_lambda, ratio=1e-2, B=500,  gamma=1000, niter=2000,
  ...
  lambda_set <- Lambda_grid(X.ori, y.ori, n_lambda, alpha=1, loss_func = "Huber", ratio)
  for (i.lambda in 1:length(lambda_set)){
    ...
    for (i.b in 1:B){
      ...
      y.sub <- y[pos,]
      X.sub <- X[pos,]
      invisible(capture.output(beta_hat_APGD <- HuberLasso_Beta(X.sub, y.sub, lambda, method="APGD",
