...
#' @param lambda0 one of parameters in Huber Lasso regression, which controls the number of nonzero coefficients.
...
#' @examples
#'
HuberLasso_Beta <- function(X, y, lambda0, method="APGD",
  ...
  if (method == "APGD"){
    ...
    for (k in 1:niter){
      ...
      z <- y - X %*% beta_prox
      obj[k+1] <-  sum(HuberFunction(z, M)) + lambda0*sum(abs(beta_prox))
    ...
    library(CVXR)
    beta11 <- Variable(p)
    obj11 <- sum(huber(y - X%*%beta11, M)) + lambda0*p_norm(beta11, 1)
