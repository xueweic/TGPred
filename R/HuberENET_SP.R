...
HuberENET_SP <- function(X, y, alphas, n_lambda, ratio=1e-2, B=500,  gamma=1000, niter=2000,
  ...
  for (i.alpha in 1:n_alpha){
    ...
    for (i.lambda in 1:length(lambda_set)){
      ...
      for (i.b in 1:B){
        ...
        y.sub <- y[pos,]
        X.sub <- X[pos,]
        invisible(capture.output(beta_hat_APGD <- HuberENET_Beta(X.sub, y.sub, lambda, alpha, method="APGD",
  ...
  SP.HuberENET <- apply(SP.LambdaAlpha, 1, max)
  print("Done with HuberENET by APGD method!")
  end.time <- proc.time()
  if (timer == TRUE){
