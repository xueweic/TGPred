...
#'
#' @examples
HuberNet_SP <- function(X, y, Adj, alphas, n_lambda, B=500,  gamma=1000, niter=2000,
  ...
  print("Start calculating selection probability using HuberNet by APGD method:")
  ...
  for (i.alpha in 1:n_alpha){
    ...
    for (i.lambda in 1:length(lambda_set)){
      ...
      for (i.b in 1:B){
        ...
        y.sub <- y[pos,]
        X.sub <- X[pos,]
        beta_hat_APGD <- HuberNet_Beta(X, y, Adj, lambda, alpha, method="APGD",
  ...
  SP.HuberNet <- apply(SP.LambdaAlpha, 1, max)
  print("Done with HuberNet by APGD method!")
  end.time <- proc.time()
  if (timer == TRUE){
