...
#' @param lambda0 one of parameters in HuberNet regression, which controls the number of nonzero coefficients.
#' @param alpha0 one of parameters in HuberNet regression, which controls the numerical values of nonzero coefficients.
#' @param method The current methods must be 'APGD' or 'CVX'
#' @param gamma initial value of gamma in APGD. default: 1000
#' @param niter the maximum number of APGD to solve HuberNet regression. default: 2000
...
#'
#' @return beta: n_genes length vector of estimated regulated effect sizes, where beta_j != 0 indicates j th gene is not selected in HuberNet regression.
#' @export
#'
#' @examples
#'
HuberNet_Beta <- function(X, y, Adj, lambda0, alpha0, method="APGD",
  ...
  if (method == "APGD"){
    ...
    for (k in 1:niter){
      ...
      z <- y - X %*% beta_k
      obj[k] <- sum(HuberFunction(z, M)) + lambda0*alpha0*sum(abs(beta_k)) + 0.5*lambda0*(1-alpha0)*t(beta_k)%*%SLS%*%beta_k
      ksi <- beta_est[,paste("beta", k)] + k/(k+3) * (beta_est[,paste("beta", k)] - beta_est[,paste("beta", k-1)])
      H_grad <- Huber_grid(y - X %*% ksi, M)
      g_grad_ksi <- - t(X) %*% H_grad + lambda0 * (1-alpha0) * SLS %*% ksi
      ...
      while (TRUE){
        ...
        g_ksi <- sum(H_ksi) + 0.5*lambda0*(1-alpha0)*t(ksi)%*%SLS%*%ksi
        H_beta_prox <- HuberFunction(y - X %*% beta_prox, M)
        g_beta_prox <- sum(H_beta_prox) + 0.5*lambda0*(1-alpha0)*t(beta_prox)%*%SLS%*%beta_prox
      ...
      z <- y - X %*% beta_prox
      obj[k+1] <-  sum(HuberFunction(z, M)) + lambda0*alpha0*sum(abs(beta_prox)) + 0.5*lambda0*(1-alpha0)*t(beta_prox)%*%SLS%*%beta_prox
    ...
    p <- ncol(X)
    beta <- Variable(p)
    obj11 <- sum(huber(y - X%*%beta, M)) + lambda0*alpha0*p_norm(beta, 1) +
